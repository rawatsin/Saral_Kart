...
const SingleProduct = () => {
  ...
  const handleAddtoCart =()=>{
    setCart(prev=>{
      const axes=prev.some(item=>item.id==data.id)
      if(axes) return prev
      return [...prev,data]
  })
          ...
          <div className="flex flex-col space-y-5">
            ...
            <div>
              ...
              <div className="mt-2 flex items-center gap-3">
                <span className="text-3xl font-bold text-slate-900 dark:text-white">
                  ₹{(data.price * 80).toLocaleString("en-IN")}
                </span>
                <span className="text-sm text-slate-400 line-through">
                  ₹{(data.price * 90).toLocaleString("en-IN")}
                </span>
                <span className="text-[11px] font-bold text-red-500 bg-red-50 dark:bg-red-950 px-1.5 py-0.5 rounded">
                  -30%
                </span>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3 pt-2">
              ...
              <button onClick={handleAddtoCart}
                className="cursor-pointer flex-1 flex items-center justify-center gap-2 bg-slate-900 dark:bg-emerald-600 text-white h-11 px-6 rounded-lg font-medium hover:opacity-90 transition-all"
              >
